    ...
    return (
        <div className="flex flex-col items-start w-[390px] xl:w-[1040px] gap-[22px] xl:gap-[40px] text-[#222]">
            ...
            <div className="w-full grid grid-cols-3 xl:grid-cols-4 gap-3">
                {/* <div className="rounded-lg shadow-md col-span-2 xl:col-span-3"> */}
                {
                    selectedTeam === null ?
                        students.map((student) => (
                            <div key={student.id} className='col-span-3 xl:col-span-2 flex flex-col items-stretch'>
                                <Image
                                    width={300}
                                    height={300}
                                    // className='w-[185px] h-[185px]  xl:w-[245px] xl:h-[245px] rounded-lg self-center mb-3'
                                    className='h-[185px]  xl:h-[245px] rounded-lg self-center mb-3'
                                    src={student.image}
                                    alt={student.name}
                                />
                                <div className="flex flex-col gap-1">
                                    <span className="font-medium text-xs xl:text-xl">姓名 {student.name}</span>
                                    <span className="text-xxs xl:text-base">學校 科系 {student.school} {student.department}</span>
                                </div>
                            </div>
                        )) :
                        students.filter(student => student.team === selectedTeam).map((student) => (
                            <div key={student.id} className='col-span-3 xl:col-span-2 flex flex-col items-stretch justify-start'>
                                <Image
                                    width={300}
                                    height={300}
                                    // className='w-[185px] h-[185px]  xl:w-[245px] xl:h-[245px] rounded-lg self-center mb-3'
                                    className='h-[185px]  xl:h-[245px] xl:w-[245px] rounded-lg self-start mb-3'
                                    src={student.image}
                                    alt={student.name}
                                />
                                <div className="flex flex-col gap-1">
